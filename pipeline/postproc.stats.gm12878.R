...
# load predictions
preds.hmm = read.table("hg19.gm12878.new_hmm2b.bed", skip = 1)
preds.post2 = read.table("hg19.gm12878.new_hmm2b.post2.bed")
...
stepcov.preds.hmm = step.coverage(preds.hmm, chroms, bwSet.gm, bwBckSet.gm, scale.factor, n.cores = n.cores)
stepcov.preds.post2 = step.coverage(preds.post2, chroms, bwSet.gm, bwBckSet.gm, scale.factor, n.cores = n.cores)

# plot step coverage information
pdf("../output/sup.s1.panelC.new.pdf", width=14, height = 3)
...
barplot(step.data, beside = T, ylim=c(0,1), names.arg = c("TAP+ > 0", "TAP+ > TAP-\n(TAP- > 0)", "TAP+ > TAP-\n(TAP- = 0)", "TAP+ < TAP-"),
        ...
        font = 2,
        lwd = 2,
        args.legend = list(x = "topright", box.lwd=2, border=NA, title = "GRO-cap HMM"))
...
n.cores = 3
chroms = levels(preds.hmm[,1])
readcov.preds.hmm = read.coverage(preds.hmm, chroms, bwSet.gm, scale.factor, n.cores = n.cores)
readcov.preds.post2 = read.coverage(preds.post2, chroms, bwSet.gm, scale.factor, n.cores = n.cores)
...
#
# load pairs
pairs.plus = read.table("hg19.gm12878.new_hmm2b.post2.pair_plus.bed")
pairs.minus = read.table("hg19.gm12878.new_hmm2b.post2.pair_minus.bed")
